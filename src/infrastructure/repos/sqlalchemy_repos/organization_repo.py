...
from src.domain import Organization, GeoPoint
from src.infrastructure.models import OrganizationORM, BuildingORM, ActivityORM, \
    organization_activities
from src.infrastructure.repos.base import BaseOrganizationRepository, BaseORMToDomainMapper
...
class OrganizationRepo(
    ...
    async def get_all_in_bbox(
        ...
        **filters,
        ...
        stmt = self._create_get_all_stmt(
            offset, limit, activity_id
        )
        ...
        stmt = stmt.where(
            func.ST_Within(
                BuildingORM.location.cast(Geometry),
                envelope
            )
        )
    ...
    async def get_all_by_activity_tree(
        ...
        base_cte = select(ActivityORM.id).where(
            ActivityORM.id == root_activity_id
        ...
        stmt = select(OrganizationORM).join(
            organization_activities,
    ...
    def _create_get_all_stmt(
        ...
        **filters: Any
        ...
        stmt = super()._create_get_all_stmt(
            offset, limit, **filters
        )

        stmt = stmt.options(
            selectinload(self.table.building),
            selectinload(self.table.activities)
        )

        if activity_id is not None:
            stmt = stmt.join(self.table.activities).where(
